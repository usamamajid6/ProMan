...
const createNewTask = async (name, description, pre_req_id, due_date, member_id_array) => {
    try {
...
const addSubTask = async (_id, subtask_id) => {
    try {
        const result = await Task.updateOne({ _id }, { $push: { sub_tasks: subtask_id } });
        return result;
    } catch (e) {
...
const addMember = async (_id, member_id) => {
    try {
        const result = await Task.updateOne({ _id }, { $push: { members: member_id } });
        return result;
    } catch (e) {
