...
const addTask = async (_id, task_id) => {
  try {
    const result = await TaskList.updateOne({ _id }, { $push: { tasks: task_id } });
    return result;
  } catch (e) {
...
const getTaskListsByProjectId = async (project_id) => {
  try {
    const result = await TaskList.find({ project: project_id }).populate("tasks").populate('tasks.task.members');
    return result;
  } catch (e) {
